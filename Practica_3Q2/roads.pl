...
writeClauses:- 
    atLeastOneGasStation,
    atMostMaxStations,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.


%% writeClause[c1,-c2,...,cn] crea una clausula en CNF -> (c1 v -c2 v ..... v cn)
% em(E,M) ^ ed(E,D) --> md(M,D) == -(em(E,M) ^ ed(E,D)) v md(M,D) == -em(E,M) v -ed(E,D) v md(M,D) CNF
% P -> Q == -P v Q
atLeastOneGasStation:- city(C1),city(C2),road(C1,C2), atLeast(1,[install(C1),install(C2)]),fail.
atLeastOneGasStation.

atMostMaxStations:- findall(install(C),city(C),Lits), maxStations(M),atMost(M,Lits),fail.
atMostMaxStations.
%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

%displaySol(M):- nl, write(M), nl, nl, fail.
displaySol(M):- 
    nl, write('Cituats amb gasolineres:'), nl, city(C), member(install(C),M),write('City '),write(C), write(' supplies cities-> '),findall(city(C1), (road(C1,C)|road(C,C1)),L), sort(L,S), write(S),
    nl, fail.
displaySol(_).
