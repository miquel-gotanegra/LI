...
city(C):-     adjacency(C,_).
road(C1,C2):- adjacency(C1,L), member(C2,L).
...
%%%%%%  1. SAT Variables:
satVariable( visited(C,P) ):-  city(C), position(P). % meaning "city C is visited in position P"
satVariable( cost(P) ):-  position(P), P>0. % meaning "the cities visited at positions P-1 and P have different parity"
...
eachCityExactlyOnePosition:-
    city(C), findall( visited(C,P), (position(P),P>0), Lits ), exactly(1,Lits), fail.
eachCityExactlyOnePosition.

eachPositionExactlyOneCity:-
    position(P), findall( visited(C,P), city(C), Lits ), exactly(1,Lits), fail.
eachPositionExactlyOneCity.

noInexistentRoads:-
    position(P2), P2>0, P1 is P2-1,  city(C1), city(C2), not( road(C1,C2) ),
    writeClause([ -visited(C1,P1), -visited(C2,P2) ]), fail.
noInexistentRoads.

defineCosts:-
    position(P2), P2>0, P1 is P2-1,  city(C1), city(C2), 1 is (C1+C2) mod 2,
    writeClause([ -visited(C1,P1), -visited(C2,P2), cost(P2) ]), fail.
defineCosts.

maxTotalCost:-
    maxCost(K), findall( cost(P), (position(P),P>0), Lits ), atMost(K,Lits), fail.
maxTotalCost.
