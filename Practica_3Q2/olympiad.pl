...
exactlyKStudents:-findall(chosen(S),student(S),Lits), numMembersTeam(K),exactly(K,Lits),fail.
exactlyKStudents.

differentTopic:- student(S),expertise(S,E),findall(chosen(S1),expertise(S1,E),Lits),atMost(1,Lits),fail.
differentTopic.

                                    %S1 \= S2,
allFriends:- student(S1),student(S2), not(friends(S1,S2)), atMost(1,[chosen(S1),chosen(S2)]),fail.
allFriends.
